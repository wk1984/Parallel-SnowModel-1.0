...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &  global_ny,byte_index_r1,byte_index_r2,nstns_max,nz_max)
      ...
      use snowmodel_vars, only: nvegtypes,n_print_vars
#ifdef NETCDF
      use netcdf_module, only: get_nearest_stns_netcdf_1,&
     &  netcdf_resolution
#elif NO_NETCDF
      use nonetcdf_module, only: get_nearest_stns_netcdf_1,&
     &  netcdf_resolution
#endif
      implicit none
     ...
     &  Tabler_1_flag,Tabler_2_flag
      ...
      real corr_factor(nx,ny,max_iter+1)
      integer icorr_factor_index(max_iter)
...
! Added variables
      integer me,np,global_ny
      integer,parameter :: k15 = selected_int_kind(15)
      integer(kind=k15) :: byte_index_r1,byte_index_r2
      real subgrid_flag,snowfall_frac
      character*80 netcdf_fpath
      real alpha_grid(nx,ny)
      integer nstns_max,nz_max
...
!   options.
      if (seaice_run.eq.3.0 .and. me.eq.0) then
...
!   in snowmodel.par.
      if (snowmodel_line_flag.eq.1.0) then
        if (ny.ne.1) then
          ...
          print *, 'nx = number of grid cells, ny = 1.'
          print *, '  The current values are:'
          print *, '    nx = ',nx,'  ny = ',ny
          stop
        endif
        if (np.gt.1 .and. me.eq.0) then
          print *,'SnowModel line simulations have not been implemented'
          print *,'  in parallel.'
          stop
        endif
      endif
...
!   make sure the rest of the model is configured correctly.
      if (snowmodel_line_flag.eq.1.0) then
        if (run_snowtran.eq.1.0 .and. seaice_run.ne.4.0 .and. me.eq.0) then
          print *, 'You cannot run snowmodel_line_flag = 1.0 with'
          print *, 'run_snowtran = 1.0'
        stop
        endif
        if (barnes_lg_domain.eq.0.0 .and. me.eq.0) then
...
!   than the model time step.
      if (multilayer_snowpack.eq.1) then
        if (tsls_threshold.lt.dt/3600.0 .and. me.eq.0) then
...
! Check the model dt value, and send an error message if dt < 3600.
      if (dt.lt.3600.0 .and. me.eq.0) then
...
! Check for condition of parallel snowmodel and subgrid models.
      if (np.gt.1 .and. subgrid_flag.gt.0.1 .and. me.eq.0) then
        print *, 'Tabler surfaces have not been implemented in'
        print *, '  parallel. If running in parallel, subgrid_flag'
        print *, '  needs to be 0.0.'
        print *, 'number of processors = ',np
        print *, 'subgrid_flag = ',subgrid_flag
        stop
      endif

! Check for condition of parallel snowmodel and Tabler output.
      if (np.gt.1 .and. Tabler_1_flag.gt.0.1 .and. me.eq.0) then
        print *, 'Tabler surfaces have not been implemented in'
        print *, '  parallel. If running in parallel, Tabler_1_flag'
        print *, '  needs to be 0.0.'
        print *, 'number of processors = ',np
        print *, 'Tabler_1_flag = ',Tabler_1_flag
        stop
      endif

      if (np.gt.1 .and. Tabler_2_flag.gt.0.1 .and. me.eq.0) then
        print *, 'Tabler surfaces have not been implemented in'
        print *, '  parallel. If running in parallel, Tabler_2_flag'
        print *, '  needs to be 0.0.'
        print *, 'number of processors = ',np
        print *, 'Tabler_2_flag = ',Tabler_2_flag
        stop
      endif
...
! Read in the topography array.
     ...
     &    form='unformatted',access='stream')
        read (37,pos=byte_index_r1) ((topo_land(i,j),i=1,nx),j=1,ny)

      elseif (ascii_topoveg.eq.1.0) then

        if (np.gt.1 .and. me.eq.0) then
          print *,'Parallel input of topography ascii files has not'
...
! Read in the vegetation array.
        ...
        if (ascii_topoveg.eq.0.0) then
          read (37,pos=byte_index_r2) ((vegtype(i,j),i=1,nx),j=1,ny)

        elseif (ascii_topoveg.eq.1.0) then

          if (np.gt.1 .and. me.eq.0) then
            print *,'Parallel input of vegetation ascii files has not'
            print *,'  been implemented.'
            stop
          endif
...
! Find the last occurance of '/' in the topo_vege path.
        ...
        if (ipath_length.eq.0 .and. me.eq.0) then
     ...
     &    form='unformatted',access='stream')
        read (191,pos=byte_index_r1) ((vegsnowd_xy(i,j),i=1,nx),j=1,ny)
        close(191)
...
!   requires it.
     ...
     &    form='unformatted',access='stream')
        read (444,pos=byte_index_r1) ((curve_wt_lg(i,j),i=1,nx),j=1,ny)
        close (444)
      endif
...
!   the SnowTran-3D two-layer submodel.
     ...
     &  run_snowtran.eq.1.0) then
        if (twolayer_flag.ne.1.0 .and. me.eq.0) then
...
!   increments that are too large.
      if (deltax.gt.500.0 .or. deltay.gt.500.0) then
        if (seaice_run.eq.0.0) then
          if (run_snowtran.eq.1.0 .and. me.eq.0) then
            print *
...
!   distributions.
     ...
     &  swed_layer,ro_layer,T_old,gamma,diam_layer,nz_max)
...
! Check to see whether the required output files will be created.
        if (print_user.ne.1.0 .and. me.eq.0) then
          print *, 'For a data assimilation run print_user must = 1.0'
          stop
        endif

        if (print_var(19).ne.'y' .and. me.eq.0) then
          print *, 'print_var_19 == y for a data assimilation run'
          stop
        endif

        if (print_var(20).ne.'y' .and. me.eq.0) then
...
!                 read(61,*) obsid(k),xstn(k),ystn(k),swe_obs(k)
          ...
          if (krec.gt.max_iter .and. me.eq.0) then
! I HAVE NOT CHANGED THE BELOW PRINT STATEMENT TO APPROPRIATELY REFLECT
!   THE REMOVAL OF max_obs_dates.
          print *
          print *, 'For a DATA ASSIMILATION RUN, MAX_OBS_DATES must be'
          print *, 'defined in SNOWMODEL.PAR to be greater than the'
          ...
          print *, 'Right now, max_obs_dates = ',max_iter
          print *
          stop
          endif
        endif
...
!   latitude weighting when calculating incoming solar radiation.
     ...
     &    form='unformatted',access='stream')
        read (91,pos=byte_index_r1) ((xlat_grid(i,j),i=1,nx),j=1,ny)
        close(91)

      elseif (lat_solar_flag.eq.1) then

        if (np.gt.1 .and. me.eq.0) then
          print *,'Parallel input of latitude ascii files has not'
          print *,'  been implemented.'
          stop
        endif
...
!   solar radiation differences from south to north.
        if (global_ny*deltay.gt.500000.0 .and. me.eq.0) then
          print *
      ...
      if (maxval(xlat_grid).gt.0.0 .and. minval(xlat_grid).lt.0.0) then
        print *
        print *, 'YOUR DOMAIN SPANS ACROSS THE EQUATOR.'
        print *, 'THE CALCULATION OF THE LAPSE RATES'
        print *, 'WITHIN THE GET_LAPSE_RATES SUBROUTINE'
        print *, 'IN MICROMET ARE LIKELY ERROR-PRONE.'
        print *, 'THE MODEL DOES NOT ALLOW FOR THIS FUNCTIONALITY.'
        print *
        stop
      endif
...
!   longitude influence when calculating incoming solar radiation.
     ...
     &    form='unformatted',access='stream')
        read (91,pos=byte_index_r1) ((xlon_grid(i,j),i=1,nx),j=1,ny)
        close(91)

      elseif (UTC_flag.eq.1.0) then

        if (np.gt.1 .and. me.eq.0) then
          print *,'Parallel input of longitude ascii files has not'
          print *,'  been implemented.'
          stop
        endif
...
!   solar radiation differences from east to west.
        if (nx*deltax.gt.500000.0 .and. seaice_run.ne.4 .and. &
     &    me.eq.0) then
          print *
          print *,'YOUR DOMAIN IS PRETTY BIG TO NOT ACCOUNT FOR'
...
! Calculate alpha_grid in conditions were xlat_grid and xlon_grid are
!   defined.
      if (UTC_flag.eq.-1.0 .and. lat_solar_flag.eq.-1.0) then
        call init_alpha_direction(nx,ny,deltax,alpha_grid,xlat_grid,&
     &    xlon_grid)
      else
        alpha_grid = 0.0
      endif
...
! Open the MicroMet station data input file.
      if (igrads_metfile.eq.1) then
        if (np.gt.1 .and. me.eq.0) then
          print *,'Parallel input of met ascii files has not'
          print *,'  been implemented.'
          stop
        endif
        open(20,file=met_input_fname,form='unformatted', &
     &    access='direct',recl=4*13)
      elseif (igrads_metfile.eq.0) then
        open (20,file=met_input_fname,form='formatted')
      endif

! Run check for forcing rain/snow partitioning only when netcdf
!   forcing is provided.
      if (snowfall_frac.gt.3.1) then
        if (igrads_metfile.ne.2 .and. me.eq.0) then
          print *
          print *,'Rain / snow partitioning from ascii or binary'
          print *,'  forcing files has not been implemented.'
          print *
          stop
        endif
      endif

! Initialize nstns_max parameter previously called from snowmodel_inc.
      if (igrads_metfile.eq.2) then
! For simulations using netcdf files the number of stations could
!   be larger than the number of grid cells.  Therefore, call this
!   function to set nstns_max equal to the total number of stations in
!   the netcdf file.
        call netcdf_resolution(netcdf_fpath,nstns_max)
      else
...
!   valid data.
      if (check_met_data.eq.1.0) then
        ...
        if (igrads_metfile.eq.2 .and. me.eq.0) then
           print *
           print *,'Met check functionality has not be implemented'
           print *, 'in wrf reader.'
           print *
           stop
        endif
...
!   nearest-station indexing array.
      if (barnes_lg_domain.eq.1.0) then
        ...
        if (igrads_metfile.eq.2) then
          call get_nearest_stns_netcdf_1(nx,ny,xmn,ymn,deltax,deltay,&
...
! If this is a history restart run, check to see if its a
!   parallel run as well.
      if (ihrestart_flag.ne.-2 .and. np.gt.1 .and. me.eq.0) then
        print *,'Parallel simulations with history restart'
        print *,'  have not been implemented.'
        stop
      endif
      ...
      if (ihrestart_flag.ge.0) then
        if (igrads_metfile.eq.0) then
          do iter=1,ihrestart_flag
            ...
            enddo
          enddo
        elseif (igrads_metfile.eq.2 .and. me.eq.0) then
          print *
          print *,'Met history restart has not been implemented for'
          print *,'  netcdf data.'
          print *
          stop
        endif
...
!   For MicroMet.
      if (run_micromet.eq.1.0 .and. print_micromet.eq.1.0) then
        ...
        if (nrecs.gt.nrecs_max .and. me.eq.0) then
     ...
     &      status='replace')
      ...
      if (print_micromet.eq.1.0 .and. np.gt.1 .and. me.eq.0) then
        print *,'Parallel output has not been implemented for'
        print *,'  print_micromet.'
        stop
      endif
...
!   For EnBal.
     ...
     &      status='replace')
      ...
      if (print_enbal.eq.1.0 .and. np.gt.1 .and. me.eq.0) then
        print *,'Parallel output has not been implemented for'
        print *,'  print_enbal.'
        stop
      endif
...
!   For SnowPack.
     ...
     &      status='replace')
      ...
      if (print_snowpack.eq.1.0 .and. np.gt.1 .and. me.eq.0) then
        print *,'Parallel output has not been implemented for'
        print *,'  print_snowpack.'
        stop
      endif
...
!   For SnowTran-3D.
     ...
     &      status='replace')
      ...
      if (print_snowtran.eq.1.0 .and. np.gt.1 .and. me.eq.0) then
        print *,'Parallel output has not been implemented for'
        print *,'  print_snowtran.'
        stop
      endif
...
!   For Multi-Layer SnowPack.
     ...
     &      recl=4*nx*ny*nz_max,status='replace')
      ...
      if (print_multilayer.gt.0.0 .and. np.gt.1 .and. me.eq.0) then
        print *,'Parallel output has not been implemented for'
        print *,'  multilayer_snowpack.'
        stop
      endif
...
! Read in the precipitation correction factor array.
     ...
     &    form='unformatted',access='stream')
        read (144,pos=byte_index_r1) ((cf_precip(i,j),i=1,nx),j=1,ny)

      elseif (cf_precip_flag.eq.2.0) then

        if (np.gt.1 .and. me.eq.0) then
          print *,'Parallel input of cp_precip has not been implemented'
          print *,'  for ascii files.'
          stop
        endif
...
!   directory called "ctl_files".
      call mk_ctl_files(nx,global_ny,deltax,deltay,xmn,ymn,dt,&
     ...
     &  multilayer_output_fname,me,nz_max)
...
!   header.
     ...
     & 'cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc'
      ...
      return
      end SUBROUTINE PREPROCESS_CODE
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &  nstns_max)
      ...
      integer nstns_max        ! maximum number of met stations
...
#ifdef SERIAL
      use serial_module, only: global_map
#elif CAF
      use caf_module, only: global_map
#endif
      implicit none
...
!   its distance a big number.
      ...
      return
      end subroutine get_nearest_stns_1
...
! Save the correction factor index.
      ...
      return
      end subroutine HRESTART_SAVE_DA
...
! Read the correction factor index.
      ...
      return
      end subroutine HRESTART_READ_DA
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &  multilayer_output_fname,me,nz_max)
      ...
      use snowmodel_vars,only: n_print_vars
      implicit none
...
!   .gdat output files that were generated by this model run.
     ...
     &    iyear_init,imonth_init,iday_init,xhour_init,max_iter,&
     &    undef,multilayer_output_fname,me,nz_max)
      endif
...
!   data files that are in the wi_assim directory.
     ...
     &      iyear_init,imonth_init,iday_init,xhour_init,max_iter,&
     &      undef,output_path,k,print_inc,me)
        endif
...
! This i2.2 puts a zero in front of single digit numbers like 1.
   ...
   60 format ('ENDVARS')
      ...
      return
      end subroutine mk_4char_vars_ctl
...
! This i2.2 puts a zero in front of single digit numbers like 1.
   ...
   68 format ('ENDVARS')
      ...
      return
      end subroutine mk_micromet_ctl
...
! This i2.2 puts a zero in front of single digit numbers like 1.
   ...
   75 format ('ENDVARS')
      ...
      return
      end subroutine mk_snowpack_ctl
...
! This i2.2 puts a zero in front of single digit numbers like 1.
   ...
   66 format ('ENDVARS')
      ...
      return
      end subroutine mk_snowtran_ctl
...
! This i2.2 puts a zero in front of single digit numbers like 1.
   ...
   68 format ('ENDVARS')
      ...
      return
      end subroutine mk_tabler_1_ctl
...
! This i2.2 puts a zero in front of single digit numbers like 1.
   ...
   60 format ('ENDVARS')
      ...
      return
      end subroutine mk_tabler_2_ctl
...
! This i2.2 puts a zero in front of single digit numbers like 1.
   ...
   67 format ('ENDVARS')
      ...
      return
      end subroutine mk_multilayer_ctl
...
! This i2.2 puts a zero in front of single digit numbers like 1.
   ...
   67 format ('ENDVARS')
      ...
      return
      end subroutine mk_multilayer_2Dxy_ctl
...
! This i2.2 puts a zero in front of single digit numbers like 1.
   ...
   67 format ('ENDVARS')
      ...
      return
      end subroutine mk_multilayer_snod_ctl
...
! This i2.2 puts a zero in front of single digit numbers like 1.
   ...
   67 format ('ENDVARS')
      ...
      return
      end subroutine mk_multilayer_sden_ctl
...
! This i2.2 puts a zero in front of single digit numbers like 1.
   ...
   67 format ('ENDVARS')
      ...
      return
      end subroutine mk_multilayer_swed_ctl
...
! This i2.2 puts a zero in front of single digit numbers like 1.
   ...
   67 format ('ENDVARS')
      ...
      return
      end subroutine mk_multilayer_diam_ctl
...
! This i2.2 puts a zero in front of single digit numbers like 1.
   ...
   67 format ('ENDVARS')
      ...
      return
      end subroutine mk_multilayer_flux_ctl
...
! This i2.2 puts a zero in front of single digit numbers like 1.
   ...
   67 format ('ENDVARS')
      ...
      return
      end subroutine mk_multilayer_temp_ctl
...
! This i2.2 puts a zero in front of single digit numbers like 1.
   ...
   67 format ('ENDVARS')
      ...
      return
      end subroutine mk_multilayer_cond_ctl

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      subroutine init_alpha_direction(nx,ny,deltax,alpha_grid,xlat_grid,&
     &  xlon_grid)

! Compute the angle of the change in longitude (alpha_grid) for each
!   SnowModel grid cell.
      implicit none

      real, parameter::deg2rad=0.017453293 !2*pi/360
      real, parameter::min_delta = 1000.0
      integer nx,ny,i,j,grids,nnl,nnr
      real alpha_grid(nx,ny),xlat_grid(nx,ny),xlon_grid(nx,ny)
      real dlon,dlat,dist,sin_alpha,alpha,rem,deltax

! Determine number of grids cells used to calculate alpha_grid.
      if (deltax.le.min_delta) then
        grids = int(min_delta / deltax)
        rem = mod(min_delta,real(deltax))
        if (rem.gt.0.1) then
          grids = grids + 1
        endif
        if (mod(grids,2).eq.0) then
          nnl = int(grids/2)
          nnr = int(grids/2)
        else
          nnl = grids/2
...
! Calculate change in longitude and latitude.
      do j = 1,ny
        do i = 1,nx
          if (i.le.nnl) then
            dlon = (xlon_grid(grids,j) - xlon_grid(1,j)) * &
     &        cos(deg2rad*(xlat_grid(i,j)))
            dlat = xlat_grid(grids,j) - xlat_grid(1,j)
          elseif (nx-i.lt.nnr) then
            dlon = (xlon_grid(nx,j) - xlon_grid(nx-grids+1,j)) * &
     &        cos(deg2rad*(xlat_grid(i,j)))
            dlat = xlat_grid(nx,j) - xlat_grid(nx-grids+1,j)
          else
            dlon = (xlon_grid(i+nnr,j) - xlon_grid(i-nnl,j)) * &
     &        cos(deg2rad*(xlat_grid(i,j)))
            dlat = xlat_grid(i+nnr,j) - xlat_grid(i-nnl,j)
          endif
! Calculate distance based on change in longitude and latitude.
          dist = sqrt((dlat)**2 + (dlon)**2)
! Calculate alpha angle.
          sin_alpha = dlat/dist
          alpha_grid(i,j) = asin(sin_alpha)/deg2rad
        enddo
      enddo

      return
      end subroutine init_alpha_direction
